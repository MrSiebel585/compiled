...
function display_usage {
    echo "Usage: $0 <source_disk> <remote_mount_point> <destination_path> <remote_host>"
...
# Read the source disk, remote mount point, destination path, and remote host from the command-line arguments
source_disk="$1"
remote_mount_point="$2"
destination_path="$3"
remote_host="$4"

# Check if the remote filesystem is already mounted
if ! mountpoint -q "$remote_mount_point"; then
    # Mount the remote filesystem using SSHFS
    sshfs "$remote_host" "$remote_mount_point" -o IdentityFile=/path/to/ssh/key
